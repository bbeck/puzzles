...
func main() {
	...
	grid.ForEachPoint(func(p aoc.Point2D, _ Cell) {
		if !loop.Contains(p) {
	...
	for x := 0; x < grid.Width; x++ {
		seen = Flood(grid, aoc.Point2D{X: x, Y: 0}, seen)
		seen = Flood(grid, aoc.Point2D{X: x, Y: grid.Height - 1}, seen)
	}
	for y := 0; y < grid.Height; y++ {
		seen = Flood(grid, aoc.Point2D{X: 0, Y: y}, seen)
		seen = Flood(grid, aoc.Point2D{X: grid.Width - 1, Y: y}, seen)
	}
...
func Triple(grid aoc.Grid2D[Cell], start aoc.Point2D) (aoc.Grid2D[Cell], aoc.Point2D) {
	next := aoc.NewGrid2D[Cell](3*grid.Width, 3*grid.Height)
	grid.ForEach(func(x int, y int, cell Cell) {
		center := aoc.Point2D{X: 3*x + 1, Y: 3*y + 1}
	...
	return next, aoc.Point2D{X: 3*start.X + 1, Y: 3*start.Y + 1}
}

func GetLoopPoints(grid aoc.Grid2D[Cell], start aoc.Point2D) aoc.Set[aoc.Point2D] {
...
func Flood(grid aoc.Grid2D[Cell], p aoc.Point2D, seen aoc.Set[aoc.Point2D]) aoc.Set[aoc.Point2D] {
	if !seen.Add(p) || !grid.GetPoint(p).IsGround() {
		return seen
	}

	grid.ForEachOrthogonalNeighbor(p, func(q aoc.Point2D, _ Cell) {
