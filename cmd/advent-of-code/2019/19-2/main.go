...
func main() {
	...
	y := sort.Search(10000, func(y int) bool {
		x := FindMinimumX(memory, y)
		return IsInTractorBeam(memory, x+99, y-99)
	})
...
func FindMinimumX(memory cpus.Memory, y int) int {
	...
	return sort.Search(x0, func(x int) bool {
		return IsInTractorBeam(memory, x, y)
	})
}

func IsInTractorBeam(memory cpus.Memory, x, y int) bool {
