...
func main() {
	...
	// Finally, use each transform to determine the position of each scanner in
	// scanner 0's coordinate system.
	var coordinates []Point3D
	for i := range scanners {
		coordinates = append(coordinates, transforms[i](Point3D{}))
	}

	// Determine the pair with the largest manhattan distance.
	var max int
	for i := 0; i < len(scanners); i++ {
		for j := i + 1; j < len(scanners); j++ {
			max = aoc.MaxInt(max, coordinates[i].ManhattanDistance(coordinates[j]))
		}
	}
	fmt.Println(max)
}
...
func (p Point3D) ManhattanDistance(other Point3D) int {
	dx := aoc.AbsInt(other.X - p.X)
	dy := aoc.AbsInt(other.Y - p.Y)
	dz := aoc.AbsInt(other.Z - p.Z)
	return dx + dy + dz
}
