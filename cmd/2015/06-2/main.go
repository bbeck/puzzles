...
type Operation func(lights [][]int, x int, y int)

var (
	On Operation = func(lights [][]int, x, y int) {
		lights[x][y]++
	}
	Off Operation = func(lights [][]int, x, y int) {
		lights[x][y]--
		if lights[x][y] < 0 {
			lights[x][y] = 0
		}
	}
	Toggle Operation = func(lights [][]int, x, y int) {
		lights[x][y] += 2
	}
)
